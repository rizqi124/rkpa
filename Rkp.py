...
def tag_all_admin(message):
    try:
        if message.chat.type not in ["supergroup", "group"]:
            bot.reply_to(message, "⚠️ Perintah ini hanya bisa dipakai di grup.")
            return
        ...
        if mentions:
            ...
            bot.send_message(message.chat.id, "❌ Tidak ada admin yang bisa ditag.")
...
def handle_sewabot(message):
    ...
    # ✅ Cek apakah grup sudah sewa aktif
    if is_sewa_aktif(chat_id):
        ...
        if paket == "1bulan" and expire:
            ...
            if sisa > 0:
                ...
                bot.reply_to(
                    message,
                    f"✅ Grup ini sudah sewa aktif (1 Bulan).\n"
                    f"⏳ Sisa {hari} hari\n"
                    f"📅 Expire: {time.strftime('%d-%m-%Y %H:%M', time.localtime(expire))}"
                )
                return
        else:
            bot.reply_to(message, "✅ Grup ini sudah sewa permanen, tidak perlu sewa lagi.")
            return

    # Kalau belum sewa → tampilkan pilihan paket
    markup = types.InlineKeyboardMarkup()
    ...
    bot.reply_to(
        ...
        f"▫️ 1 Bulan: Rp{HARGA_SEWA['1bulan']:,}".replace(",", ".") + "\n"
        f"▫️ Permanen: Rp{HARGA_SEWA['permanen']:,}".replace(",", "."),
        reply_markup=markup
    )
...
def handle_paket(call):
    ...
    caption = (
        f"✅ Paket *{paket}* dipilih.\n\n"
        "💳 Silakan scan QRIS berikut untuk pembayaran.\n\n"
        f"💰 Harga: Rp{HARGA_SEWA[paket]:,}".replace(",", ".") + "\n\n"
        "📌 Setelah bayar, kirim bukti pembayaran (foto/document) "
...
def rekap_handler(msg):
    ...
    if team1_total == team2_total:
        ...
        teks2 = (
            "𝐂𝐄𝐊 𝐍𝐈𝐊 𝐌𝐀𝐒𝐈𝐍𝐆 𝐌𝐀𝐒𝐈𝐍𝐆,𝐎𝐓𝐖 𝐑𝐎𝐋,𝐉𝐀𝐍𝐆𝐀𝐍 𝐋𝐔𝐏𝐀 "
            "𝐂𝐄𝐊 𝐋𝐀𝐘𝐀𝐑 𝐀𝐌𝐀𝐍 𝐆𝐀,𝐋𝐎𝐒𝐄 𝐍𝐎 𝐃𝐑𝐀𝐌𝐀,𝐃𝐑𝐀𝐌𝐀 𝐁𝐀𝐖𝐀 "
            "𝐁𝐔𝐊𝐓𝐈(𝐑𝐄𝐂/𝐒𝐒),𝐄𝐍𝐉𝐎𝐘 𝐓𝐇𝐄 𝐆𝐀𝐌𝐄."
        )
